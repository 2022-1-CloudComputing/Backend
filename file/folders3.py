...
# S3 resource 생성
def get_s3_resource(access_key_id, secret_access_key, session_token=None):
    return boto3.resource(
        "s3", aws_access_key_id=access_key_id, aws_secret_access_key=secret_access_key, aws_session_token=session_token,
    )


# S3 URL 생성
def get_s3_url(path):
    return "https://{0}.s3.amazonaws.com/{1}".format(AWS_STORAGE_BUCKET_NAME, path)
...
def upload_folder(s3_client, folder):
    try:
        response = s3_client.put_object(Bucket=AWS_STORAGE_BUCKET_NAME, Key=folder, ACL="public-read")
        return response
    except Exception as e:
        print("Error on line {}".format(sys.exc_info()[-1].tb_lineno), type(e).__name__, e)
...
def rename_move_folder(s3_client, target, new_key):
    try:
        s3_client.copy_object(
            ...
            Key=new_key,
            CopySource={"Bucket": AWS_STORAGE_BUCKET_NAME, "Key": target},
            ACL="public-read",
        )

        # 내부 컨텐츠 이동
        contents = s3_client.list_objects(Bucket=AWS_STORAGE_BUCKET_NAME, Prefix=target, Delimiter="/")
        if "CommonPrefixes" in contents:
            for content in contents["CommonPrefixes"]:
                ...
                s3_client.copy_object(
                    Bucket=AWS_STORAGE_BUCKET_NAME,
                    Key=new_path,
                    CopySource={"Bucket": AWS_STORAGE_BUCKET_NAME, "Key": old_path},
                    ACL="public-read",
                )
...
def clear_folder(s3_client, folder):
    try:
        # 내부 컨텐츠 이동
        contents = s3_client.list_objects(Bucket=AWS_STORAGE_BUCKET_NAME, Prefix=folder, Delimiter="/")
        for content in contents["CommonPrefixes"]:
            old_path = content["Prefix"]
            s3_client.delete_object(Bucket=AWS_STORAGE_BUCKET_NAME, Key=old_path)
    except Exception as e:
